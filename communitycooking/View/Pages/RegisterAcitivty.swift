...
struct RegisterAcitivty: View {
    
    @EnvironmentObject private var navigationStack: NavigationStack
    
    @ObservedObject private var viewController = RegisterViewController()
        ...
        ZStack(alignment: .center) {
            ...
            Group {
                ...
                VStack(spacing: 20) {
                    ...
                    TextFieldUsernameValidation(text: self.$viewController.username, label: "Benutzername")
                    ...
                    TextFieldPasswordValidation(text: self.$viewController.passwordRepeat, label: "Passwort wiederholen")
                    
                    
                    Text("Bereits ein Konto?")
                        .modifier(CommunityCookingTheme.typography.body1)
                        .onTapGesture {
                            self.onRedirectToLoginActivityClicked()
                        }
                        
                    Spacer().frame(height: 8)
                    
                    Button(action: onRegisterClicked) {
                        Text("Registrieren".uppercased())
    ...
    func onRegisterClicked() {
        viewController.onRegisterClicked()
    }
    
    func onRedirectToLoginActivityClicked() {
        navigationStack.pop()
        navigationStack.push(LoginAcitivty())
    }
}
